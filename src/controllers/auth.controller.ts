...
export class AuthController {
  ...
  register = async (req: Request, res: Response): Promise<void> => {
    const response = await this.authService.register(req.body, this.getSessionMetadata(req))
    res.status(201).json(response)
  }

  login = async (req: Request, res: Response): Promise<void> => {
    const response = await this.authService.login(req.body, this.getSessionMetadata(req))
    res.json(response)
  }
